...
SRC_W=`ffprobe $SRC_FILE |& grep -o '[0-9]\{2,4\}x[0-9]\{2,4\}' | grep -o '[0-9]\{1,4\}x' | sed 's/x//'`
SRC_H=`ffprobe $SRC_FILE |& grep -o '[0-9]\{2,4\}x[0-9]\{2,4\}' | grep -o 'x[0-9]\{1,4\}' | sed 's/x//'`
...
if [ $SRC_W -lt $SRC_H ]; then
	...
	CROP_H=$( ($DST_H / $DST_W) * $CROP_W )
    # If aspect ratio too wide #
    elif [ $($SRC_W / $SRC_H) -gt $($DST_W / $DST_H) ]; then
	CROP_W=$( ($DST_W / $DST_H) * $CROP_H )
    fi 

    ffmpeg -i -vf "crop=${CROP_W}:${CROP_H},scale=${DST_W}:${DST_H}" $SRC_FILE -s $DST_Wx$DST_H -an -f image2 -qmax 1 -qmin 1 "${DST_PATH}/frame_%05d.jpg"
    ...
    ffmpeg -i $SRC_FILE -vf "crop=${CROP_W}:${CROP_H},scale=${DST_W}:${DST_H}" -an -f image2 -qmax 1 -qmin 1 "${DST_PATH}/frame_%05d.jpg"
